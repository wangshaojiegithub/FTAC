...
load('BA_matrices.mat');                % 将已有的数据导入工作空间
load('BAnet_AveLengthofLMLs.mat');


%  ************************** (1) 首先,产生一个100节点的BA网络,并展示效果 *********************************

%   Num--顶点个数，Per--连接概率，avg_length--边的平均长度
Num          = 100;
matrix_100 = func_scalefree_network(Num);
...
title('Statistics on degree distribution of BA network with 100 nodes'); 
...
G = digraph(matrix_100');        %   绘制网络图函数的矩阵元素(i,j)表示的是i到j有没有边,所以要对 A 转置一下
figure(2);  p_graph=plot(G,'Layout','force','EdgeColor',[0 0.4 0.8],'LineWidth',0.5,'NodeColor','k','NodeFontSize',8);   % 绘制权值图 

[average_loopslength,MaximumLoopLength] = func_averageLML(G,matrix_100,p_graph);      % 计算平均LML,并在计算过程中用红色边标出最大的loop
disp(['100节点的BA网络的LML长度为：',num2str(MaximumLoopLength)]); 
disp(['100节点的BA网络的LMLs平均长度为：',num2str(average_loopslength)]); 
...
for n=50:50:1000
    Num          = n;
    for ni=1:20
        Adja_matrix  = func_scalefree_network(Num);
        while true
        ...
        eval(['BA_matrices.BA_matrices_of_',num2str(Num),'_nodes.A',num2str(ni),'=Adja_matrix']);       
    end    
end
save ('BA_matrices','BA_matrices');     % 所有数据保存到一个结构体



% ************************ (3) 导入保存的邻接矩阵数据 BA_matrices.mat, 完成不同规模网络的相关统计 ****************************
load('BA_matrices.mat');
load('BAnet_AveLengthofLMLs.mat');

for n=50:50:1000
    Num          = n;
    for ni=1:20
        eval(['matrix_temp=BA_matrices.BA_matrices_of_',num2str(Num),'_nodes.A',num2str(ni)]); % 相当于matrix_temp=BA_matrices.BA_matrices_of_200_nodes.A2;
        ...
        eval(['BAnet_AveLengthofLMLs.BAnet_AveLengthofLMLs_',num2str(Num),'_nodes.A',num2str(ni),'=AverageLength_of_LMLs']);
