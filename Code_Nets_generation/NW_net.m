clear;clc; close all;
warning off;
% rng(1);   % 控制随机数生成器,用于使每次实验生成的随机数一样,从而做一些跟踪研究

load('NW_matrices.mat');    % 将已有的数据导入工作空间
load('NWnet_AveLengthofLMLs.mat');


%  **************************  (1) 首先,产生一个100节点的NW网络,并展示效果 *********************************

%   Num--顶点个数，Per--增加新边的概率，avg_length--边的平均长度
Num          = 100;
K            = 2;
Per          = 0.005;
...
title('Statistics on degree distribution of NW network with 100 nodes'); 
subplot(212);   bar([0:M],P_Dds,'r');  xlabel('The degree of node');    ylabel('Probability of node degree');
 
% ************************************ 对原来对称的NW图进行裁边 *******************************************

while true
    matrix_100_temp=matrix_100;
    for i=1:Num 
        for j=(i+1):Num
            if matrix_100_temp(i,j)~=0 && rand()<0.1
               matrix_100_temp(i,j)=0;               % 以概率0.1删除原图邻接矩阵中上三角的一些边
            end
        end
    end
    % 确保随机删除连边后的图仍然是连通的
            Gtemp = digraph(matrix_100_temp');            % 注意 画图函数的矩阵元素(i,j)表示的是i到j有没有边,所以要对 A 转置一下
            dist = distances(Gtemp)';       % 返回一个矩阵dist,其元素d(i,j)表示所有节点对的最短路径距离(vj到vi的最短路径) 
    L=diag(sum(matrix_100_temp,2))-matrix_100_temp; eigenvalues=eig(L);  
    eigenvalues_003=roundn(eigenvalues,-3);   
    if length(find(eigenvalues_003==0))==1 && isempty(dist(dist==inf))
        dist=[]; Gtemp=[];
        matrix_100=matrix_100_temp;
        break;
    end
end
...
disp(['100节点的NW网络的LML长度为：',num2str(MaximumLoopLength)]); 
disp(['100节点的NW网络的LMLs平均长度为：',num2str(average_loopslength)]); 



% % **********************  (2)  网络规模设定：50,100,150,...,950,1000。每个网络规模下随机生成20个网络以观察平均表现 *********************
for n=50:50:1000
    Num          = n;
    for ni=1:20
        ...
        eval(['NW_matrices.NW_matrices_of_',num2str(Num),'_nodes.A',num2str(ni),'=Adja_matrix']);       
    end    
end
save ('NW_matrices','NW_matrices');     % 所有数据保存到一个结构体



% ************************ (3) 导入保存的邻接矩阵数据 BA_matrices.mat, 完成不同规模网络的相关统计 ****************************
load('NW_matrices.mat');
load('NWnet_AveLengthofLMLs.mat');

for n=50:50:1000
    Num          = n;
    for ni=1:20
        eval(['matrix_temp=NW_matrices.NW_matrices_of_',num2str(Num),'_nodes.A',num2str(ni)]); % 相当于matrix_temp=BA_matrices.BA_matrices_of_200_nodes.A2;
        ...
        eval(['NWnet_AveLengthofLMLs.NWnet_AveLengthofLMLs_',num2str(Num),'_nodes.A',num2str(ni),'=AverageLength_of_LMLs']);
%         eval(['NWnet_MaxLoopLength.NWnet_MaxLoopLength_',num2str(Num),'_nodes.A',num2str(ni),'=MaxLoopLength']);
        
        save ('NWnet_AveLengthofLMLs','NWnet_AveLengthofLMLs');
%         save ('NWnet_MaxLoopLength','NWnet_MaxLoopLength');
    end
end
