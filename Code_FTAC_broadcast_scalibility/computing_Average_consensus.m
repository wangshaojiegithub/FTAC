...
%********************** 在vi处计算标准的di,delta_i向量 **********************
    ...
    while i<=size(O_delta_matrix,1)
        ...
        if ~isempty(find(AAA_temp==inf)) || ~isempty(isnan(AAA_temp))
            AAA_temp(isnan(AAA_temp))=0;
            AAA_temp(find(AAA_temp==inf))=0;        %将可能的inf和nan置零
        end
...
%                 delta_i_vec=null(pi_delta_FDH(1:ri+1,1:ri+1),'r')';       % ***注意:写成pi_delta_FDH(1:ri+1,1:ri+1)而不是pi_delta_FDH
                ...
                % *************  替换成数值稳定性更好的 null函数 *************
                delta_i_vec=null(pi_delta_FDH(1:ri+1,1:ri+1))';
                if ~isempty(delta_i_vec)
                NormalizingNumber=1/delta_i_vec(end);  
                delta_i_vec=delta_i_vec*NormalizingNumber;
                end
                % *********************************************************
...
%                                 delta_i_vec=null(pi_delta_FDH(1+downsliding:ri+1+downsliding,1+rightslideing:ri+1+rightslideing),'r')';
                                ...
                                % *************  替换成数值稳定性更好的 null函数 *************
                                delta_i_vec=null(pi_delta_FDH(1+downsliding:ri+1+downsliding,1+rightslideing:ri+1+rightslideing))';
                                if ~isempty(delta_i_vec)
                                NormalizingNumber=1/delta_i_vec(end);  
                                delta_i_vec=delta_i_vec*NormalizingNumber;
                                end
...
%                 delta_i_vec=null(qi_delta_FDH(1:ri+1,1:ri+1),'r')';   
                ...
                % *************  替换成数值稳定性更好的 null函数 *************
                delta_i_vec=null(qi_delta_FDH(1:ri+1,1:ri+1))';
                if ~isempty(delta_i_vec)
                NormalizingNumber=1/delta_i_vec(end);  
                delta_i_vec=delta_i_vec*NormalizingNumber;
                end
...
%                                 delta_i_vec=null(qi_delta_FDH(1+downsliding:ri+1+downsliding,1+rightslideing:ri+1+rightslideing),'r')';
                                ...
                                % *************  替换成数值稳定性更好的 null函数 *************
                                delta_i_vec=null(qi_delta_FDH(1+downsliding:ri+1+downsliding,1+rightslideing:ri+1+rightslideing))';
                                if ~isempty(delta_i_vec)
                                NormalizingNumber=1/delta_i_vec(end);  
                                delta_i_vec=delta_i_vec*NormalizingNumber;
                                end
...
%                             delta_temp=null(pi_delta_FDH(1:ri+1,1:ri+1),'r')';
                            ...
                            % *************  替换成数值稳定性更好的 null函数 *************
                            delta_temp=null(pi_delta_FDH(1:ri+1,1:ri+1))';
...
%                             delta_temp=null(qi_delta_FDH(1:ri+1,1:ri+1),'r')';
                            ...
                            % *************  替换成数值稳定性更好的 null函数 *************
                            delta_temp=null(qi_delta_FDH(1:ri+1,1:ri+1))';
                            if ~isempty(delta_temp)
                            NormalizingNumber=1/delta_temp(end);  
                            delta_temp=delta_temp*NormalizingNumber;
                            end
