...
load('BA_matrices.mat');DatasetNumber=3;
...
for Num=100:50:500
    for ni=1:1
        ...
        if DatasetNumber==3
            eval(['A_temp=BA_matrices.BA_matrices_of_',num2str(Num),'_nodes.A',num2str(ni)]);
        end
...
%             W=0.5*eye(size(A_temp,1))+0.5*A_weight;
        ...
        %         eval(['S1_LeftVecUni_at_v',num2str(vi),'=LeftVecUni_at_vi']);      
            ...
            if DatasetNumber==3
                ...
                if miu==1
                    eval(['Output_BAnet_Stableweight_S1_MaxTimestep.S1_MaxTimestep_of_',num2str(Num),'_nodes.MT',num2str(ni),'=S1_MaxTimestep']);
                    eval(['Output_BAnet_Stableweight_S1_NodeFailRate.S1_NodeFailRate_of_',num2str(Num),'_nodes.NFR',num2str(ni),'=S1_NodeFailRate']);
                    save('Output_BAnet_Stableweight_S1_MaxTimestep','Output_BAnet_Stableweight_S1_MaxTimestep');
                    save('Output_BAnet_Stableweight_S1_NodeFailRate','Output_BAnet_Stableweight_S1_NodeFailRate');  
                else
                    eval(['Output_BAnet_DWS1weight_S1_MaxTimestep.S1_MaxTimestep_of_',num2str(Num),'_nodes.MT',num2str(ni),'=S1_MaxTimestep']);
                    eval(['Output_BAnet_DWS1weight_S1_NodeFailRate.S1_NodeFailRate_of_',num2str(Num),'_nodes.NFR',num2str(ni),'=S1_NodeFailRate']);
                    save('Output_BAnet_DWS1weight_S1_MaxTimestep','Output_BAnet_DWS1weight_S1_MaxTimestep');
                    save('Output_BAnet_DWS1weight_S1_NodeFailRate','Output_BAnet_DWS1weight_S1_NodeFailRate');  
                end
            end

            clearvars -except BA_matrices DatasetNumber Num ni W LeftVecUni_real_at_vi S1_di_vec S1_ri_max_vec Output_BAnet_DWS1weight_S1_MaxTimestep Output_BAnet_DWS1weight_S1_NodeFailRate S1_MaxTimestep S1_NodeFailRate
          % ************************************ Stage1 分布式终止程序 ****************************************
            D=diag(diag(W));
        ...
        %         end     
             ...
             if DatasetNumber==3
                ...
                if miu_new==1
                    eval(['Output_BAnet_Stableweight_S2_MaxTimestep.S2_MaxTimestep_of_',num2str(Num),'_nodes.MT',num2str(ni),'=S2_MaxTimestep']);
                    eval(['Output_BAnet_Stableweight_S2_NodeFailRate.S2_NodeFailRate_of_',num2str(Num),'_nodes.NFR',num2str(ni),'=S2_NodeFailRate']);
%                     save('Output_BAnet_Stableweight_S2_MaxTimestep','Output_BAnet_Stableweight_S2_MaxTimestep');
                    save('Output_BAnet_Stableweight_S2_NodeFailRate','Output_BAnet_Stableweight_S2_NodeFailRate');  
...
%                     S1S2_NodeFailRate=S1_NodeFailRate+S2_NodeFailRate;              % 两个阶段整体的节点失败率,是Stage 1和Stage 2各自的节点失败率之和
                    ...
                    eval(['Output_BAnet_Stableweight_S1S2_MaxTimestep.S1S2_MaxTimestep_of_',num2str(Num),'_nodes.MT',num2str(ni),'=S1S2_MaxTimestep']);
                    eval(['Output_BAnet_Stableweight_S1S2_NodeFailRate.S1S2_NodeFailRate_of_',num2str(Num),'_nodes.NFR',num2str(ni),'=S1S2_NodeFailRate']);   
%                     save('Output_BAnet_Stableweight_S1S2_MaxTimestep','Output_BAnet_Stableweight_S1S2_MaxTimestep');
                    save('Output_BAnet_Stableweight_S1S2_NodeFailRate','Output_BAnet_Stableweight_S1S2_NodeFailRate');
                else
                    eval(['Output_BAnet_DWS1weight_S2_MaxTimestep.S2_MaxTimestep_of_',num2str(Num),'_nodes.MT',num2str(ni),'=S2_MaxTimestep']);
                    eval(['Output_BAnet_DWS1weight_S2_NodeFailRate.S2_NodeFailRate_of_',num2str(Num),'_nodes.NFR',num2str(ni),'=S2_NodeFailRate']);
%                     save('Output_BAnet_DWS1weight_S2_MaxTimestep','Output_BAnet_DWS1weight_S2_MaxTimestep');
                    save('Output_BAnet_DWS1weight_S2_NodeFailRate','Output_BAnet_DWS1weight_S2_NodeFailRate');  

                    S1S2_MaxTimestep=max(S1_MaxTimestep,S2_MaxTimestep);            % 两个阶段整体的最大时间步,是取Stage 1和Stage 2最大时间步的最大值
%                     S1S2_NodeFailRate=S1_NodeFailRate+S2_NodeFailRate;              % 两个阶段整体的节点失败率,是Stage 1和Stage 2各自的节点失败率之和
                    S1S2_NodeFailRate=max(S1_NodeFailRate,S2_NodeFailRate);            % 若阶段1的失败率是1,那么整体的失败率是1,因此不是求和 
                    eval(['Output_BAnet_DWS1weight_S1S2_MaxTimestep.S1S2_MaxTimestep_of_',num2str(Num),'_nodes.MT',num2str(ni),'=S1S2_MaxTimestep']);
                    eval(['Output_BAnet_DWS1weight_S1S2_NodeFailRate.S1S2_NodeFailRate_of_',num2str(Num),'_nodes.NFR',num2str(ni),'=S1S2_NodeFailRate']);   
%                     save('Output_BAnet_DWS1weight_S1S2_MaxTimestep','Output_BAnet_DWS1weight_S1S2_MaxTimestep');
                    save('Output_BAnet_DWS1weight_S1S2_NodeFailRate','Output_BAnet_DWS1weight_S1S2_NodeFailRate');           
            ...
            if DatasetNumber==3
                ...
                if miu_new==1
                    ...
                    eval(['Output_BAnet_Stableweight_S2_asyn_TerminalTime.S2_asyn_TerminalTime_of_',num2str(Num),'_nodes.TT',num2str(ni),'=S2_asyn_TerminalTime']);
                    save('Output_BAnet_Stableweight_S2_asyn_TerminalTime','Output_BAnet_Stableweight_S2_asyn_TerminalTime');

                else
                    S1S2_syn_TerminalTime=max(S1_vi_stop_at_t_vec)+max(S2_vi_stop_at_t_vec);
                    eval(['Output_BAnet_DWS1weight_S1S2_syn_TerminalTime.S1S2_syn_TerminalTime_of_',num2str(Num),'_nodes.TT',num2str(ni),'=S1S2_syn_TerminalTime']);
                    save('Output_BAnet_DWS1weight_S1S2_syn_TerminalTime','Output_BAnet_DWS1weight_S1S2_syn_TerminalTime');
                end
            end
