function [vi_ready_at_t_vec,vi_stop_at_t_vec]=synchronized_termination(W,dist,epsi_max,S1_ri_max_vec)

t=0;          t_final=3*size(W,1);                   % 注意：t是从0开始
...
vi_ready_at_t_vec = zeros(size(W,1),1);
vi_stop_at_t_vec =  zeros(size(W,1),1);
    ...
    for  vi=1:size(W,1)
        ...
        elseif t>=2*(S1_ri_max_vec(vi)+1)-1 & count_vi_vec(vi)~=-1
            if t==2*(S1_ri_max_vec(vi)+1)-1
                vi_ready_at_t_vec(vi)=t;        % 记录vi准备停止的时间,就是完成本地计算的时刻t=2(ri_max+1)-1
            end 
            ...
            if (count_vi_vec(vi)==-1 & vi_stop_at_t_vec(vi)==0)
                vi_stop_at_t_vec(vi)=t;                  % 记录vi停止的时刻
            end
        end  
    end   
